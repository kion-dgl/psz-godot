...
// A storage row. Gear (weapon/armor/unit/mag) carries a rarity + stats;
// consumables carry an effect + a count. The detail panel renders one or the
// other. Rarity stars live in the DETAILS, not the list name; equipped gear is
// marked with an [E] PREFIX (consistent with the shop rows).
type StorageItem = {
...
const INVENTORY: StorageItem[] = [
  { name: 'Saber +3', rarity: '★★★★', qty: '1', equipped: true, kind: 'weapon',
  ...
  { name: 'Mag', rarity: '★', qty: '1', equipped: true, kind: 'mag',
...
const STORAGE: StorageItem[] = [
  ...
  { name: 'Heat Resist Lv1', rarity: '★★', qty: '1', kind: 'unit',
        ...
        <DetailPanel>
          {isItems ? (
            item.kind === 'item' ? (
              ...
              // Gear — rarity (stars) then stats.
              <>
                {item.rarity && <div style={{ fontSize: 14, fontWeight: 700, color: '#e0a020', marginBottom: 6 }}>{item.rarity}</div>}
