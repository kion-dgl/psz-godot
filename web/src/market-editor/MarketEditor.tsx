...
const ITEM_CART_KEY = 'market-editor-item-cart-transform-v1';
const SELECTED_CART_KEY = 'market-editor-selected-cart-v1';
...
function loadCartTransform(): CartTransform {
  return loadTransformFromKey(CART_KEY, DEFAULT_CART_TRANSFORM);
}

function loadItemCartTransform(): CartTransform {
  return loadTransformFromKey(ITEM_CART_KEY, DEFAULT_ITEM_CART_TRANSFORM);
}

function loadTransformFromKey(key: string, fallback: CartTransform): CartTransform {
  try {
...
function loadSelectedCart(): CartId {
  const v = localStorage.getItem(SELECTED_CART_KEY);
  return v === 'item' ? 'item' : 'weapon';
}
...
export default function MarketEditor() {
  ...
  const [itemCartTransform, setItemCartTransform] = useState<CartTransform>(loadItemCartTransform);
  const [selectedCart, setSelectedCart] = useState<CartId>(loadSelectedCart);
  ...
  useEffect(() => {
    localStorage.setItem(ITEM_CART_KEY, JSON.stringify(itemCartTransform));
  }, [itemCartTransform]);
  useEffect(() => {
    localStorage.setItem(SELECTED_CART_KEY, selectedCart);
  }, [selectedCart]);
  ...
  const resetCart = useCallback(() => {
    if (selectedCart === 'item') setItemCartTransform(DEFAULT_ITEM_CART_TRANSFORM);
    else setCartTransform(DEFAULT_CART_TRANSFORM);
  }, [selectedCart]);
  ...
  const placeSnippet = useMemo(() => {
    const cartBlock = (
      ...
      return [
        `## ${title}`,
        `const ${constName} := preload("${resPath}")`,
        ``,
        `func ${funcName}() -> void:`,
        `\tvar cart: Node3D = ${constName}.instantiate()`,
    ...
    const weapon = cartBlock(
      'Weapon-shop replacement cart. Generated by /market-editor.',
      'WEAPON_SHOP_CART_SCENE',
      '_add_weapon_shop_cart',
      'res://assets/stages/city_e/market/weapon_shop/weapon_shop_cart.glb',
      cartTransform,
    );
    const item = cartBlock(
      'Item-shop cart. Generated by /market-editor.',
      'ITEM_SHOP_CART_SCENE',
      '_add_item_shop_cart',
      'res://assets/stages/city_e/market/item_shop/item_cart.glb',
      itemCartTransform,
    );
    return [
  ...
  }, [cartTransform, itemCartTransform]);
    ...
    <div style={{ display: 'flex', height: '100%' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <MarketCanvas
          ...
          itemCartTransform={itemCartTransform}
          selectedCart={selectedCart}
          transformMode={transformMode}
          onCartChange={setCartTransform}
          onItemCartChange={setItemCartTransform}
      ...
      >
        ...
        {mode === 'place' ? (
          <PlacePanel
            ...
            transform={selectedCart === 'item' ? itemCartTransform : cartTransform}
            onChange={selectedCart === 'item' ? setItemCartTransform : setCartTransform}
            transformMode={transformMode}
  ...
  return (
    <>
      <p style={{ margin: 0, fontSize: 12, color: '#888' }}>
        Drag the gizmo to position the selected cart. Switch which cart the
        gizmo is attached to with the toggle below.
        Hotkeys: <kbd style={kbdStyle}>G</kbd> translate · <kbd style={kbdStyle}>R</kbd> rotate ·{' '}
        <kbd style={kbdStyle}>S</kbd> scale.
      </p>

      <div>
        <div style={{ fontSize: 11, color: '#888', marginBottom: 4 }}>Selected cart</div>
        <div
          style={{
            display: 'flex',
            background: '#0e0e22',
        ...
        >
          {([
            { id: 'weapon' as const, label: 'Weapon shop' },
            { id: 'item' as const, label: 'Item shop' },
          ]).map(({ id, label }) => (
            <button
              key={id}
              onClick={() => setSelectedCart(id)}
              style={{
                flex: 1,
                background: selectedCart === id ? '#3a3a66' : 'transparent',
                color: selectedCart === id ? '#fff' : '#888',
                border: 'none',
                borderRadius: 3,
                padding: '6px 0',
                fontSize: 11,
                fontWeight: selectedCart === id ? 600 : 400,
                cursor: 'pointer',
              }}
            >
              {label}
