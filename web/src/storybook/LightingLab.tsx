...
/**
 * Find the lantern-head anchor of every lamp instance in a room.
 *
 * Lamps are the mesh whose material is NAMED `…lamp…` — snowfield's
 ...
 * filename, which dragged the authored fire billboards — treetops included
 * — into the anchor set.
 *
 ...
 * one lamp — a post (~5 units) or a perimeter tower (~30). Measured against
 * the rooms: sa1→18, na1→20, ic1→7, ga1→4.
 *
 ...
 * Keep the posts, lose the forest.
 *
 ...
 */
function findLampFires(root: THREE.Object3D): THREE.Vector3[] {
  const CELL = 2;
  ...
  const cand = [...cells.values()].filter((c) => c.maxY >= MIN_HEIGHT);
  ...
  return [...regions.values()]
    .filter((r) => r.n >= MIN_VERTS && r.top <= MAX_TOP)
...
function Room({ def, mode, ctrl, onReady }: RoomProps) {
  ...
  useEffect(() => {
    ...
    const lamps = findLampFires(prepared.node);
