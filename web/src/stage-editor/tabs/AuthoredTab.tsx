 ...
 * generator. The panel leads with the LAYOUTS row (a room's real
 * arrangements) because that is the question this tab answers; the "table"
 * view (every record at once) and the prose that used to explain all this
 * live behind tooltips on the controls themselves.
 *
...
interface Props {
  ...
  /** 'layout' = one real arrangement; 'table' = every record at once. */
  view: 'layout' | 'table';
  setView: (v: 'layout' | 'table') => void;
...
export default function AuthoredTab({
  ...
  // A room on 'layout' view with no explicit pick opens on its most-taken
  // arrangement — intent first, the table is the opt-out.
  useEffect(() => {
    if (view !== 'layout' || layoutMask != null) return;
  ...
  }, [view, layoutMask, entry, masks, eligible, weightsByDepth, setLayoutMask]);
