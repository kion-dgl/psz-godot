...
# Optional: if dbus-run-session isn't installed, run without it (and warn) so a
# missing dep doesn't masquerade as a game failure.
DBUS_WRAP=""
if command -v dbus-run-session >/dev/null 2>&1; then
	DBUS_WRAP="dbus-run-session --"
else
	echo "[record-boot] WARN: dbus-run-session not found — running without a private DBus session; godot may SIGABRT if no ambient a11y bus is present." >&2
fi
