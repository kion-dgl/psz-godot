...
const WEAPON_TYPE_CONFIGS := {
	...
	13: {  # LASER_CANNON — single heavy beam (l_cannon source has only atk1)
		...
		"damage_mult": [2.2],
		"speed_mult": 0.55,
		"hitbox_size": Vector3(2.5, 2.0, 6.0),
		"hitbox_offset": 5.0,
		"knockback": [12.0],
		"combo_window": 0.7,
		"hit_h_dist": 10.0,
...
# Per-weapon-type WeaponComboConfig resources: the chain-accept boundary as a
# FRACTION of the current swing, indexed by the step being chained FROM.
# Untuned types share the default config; tuned types get their own .tres so
# playtest adjustments are data edits, not code.
...
## Chain-accept boundary for chaining FROM `from_step` (1-based combo_state).
