...
##
## Priority order:
##   1. Direct line of sight (floor-sample raycast clears).
##   2. Authored waypoints + edges from unified-stage-configs.json for the
##      current cell's stage_id (placed in the waypoint editor by the user
##      OR generated by scripts/tools/generate_waypoints.ts).
##   3. BFS over computed safe-spawn waypoints + center (fallback when
##      nothing is authored for this stage).
##
## Direct-first matters because the generated graphs add corner waypoints
## even to simple stages (1 portal + clear floor). For those stages, BFS
## would route the player around a corner unnecessarily and stall on the
## detour. The floor-sample raycast reliably says "clear" for those
## straight-line cases and "blocked" for L-bends, so it's the right gate.
func _find_walk_path(field: Node, portal_data: Dictionary, target: Vector3) -> Array:
	...
	if _raycast_walkable(start, target):
		return [target]
	# (2) Authored waypoints for this stage.
	var stage_id := ""
